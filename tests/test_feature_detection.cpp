...
TEST_F(FeatureDetectorTest, SetBlockSize_ActuallyChangesValue) {
    // Create a detector and set block_size to 5
    detector.set_block_size(5);
    
    // Verify by calling detect_features and checking behavior
    // When block_size changes, goodFeaturesToTrack uses different window size
    std::vector<cv::Point2f> points;
    int count1 = detector.detect_features(test_image, points);
    
    // Change block_size to different value
    detector.set_block_size(15);
    std::vector<cv::Point2f> points2;
    int count2 = detector.detect_features(test_image, points2);
...
TEST_F(FeatureDetectorTest, SetKsize_ActuallyChangesValue) {
    detector.set_ksize(5);
    
    std::vector<cv::Point2f> points;
    detector.detect_features(test_image, points);
    
    detector.set_ksize(7);
    std::vector<cv::Point2f> points2;
    detector.detect_features(test_image, points2);
    
    SUCCEED() << "set_ksize executed without crash";
}

// Test boundary values for set_block_size
TEST_F(FeatureDetectorTest, SetBlockSize_BoundaryValues) {
    ...
    std::vector<cv::Point2f> points;
    EXPECT_NO_THROW(detector.detect_features(test_image, points));
}

// Test that set_block_size clamps invalid values to valid range [1, 31]
TEST_F(FeatureDetectorTest, SetBlockSize_ClampsToRange) {
    // Test clamping of 0 to 1 (lower bound)
    detector.set_block_size(0);
    std::vector<cv::Point2f> points1;
    int count1 = detector.detect_features(test_image, points1);
    EXPECT_GE(count1, 0);
    ...
    int count3 = detector.detect_features(test_image, points3);
    EXPECT_GE(count3, 0);
    
    // Test clamping of large value to 31
    detector.set_block_size(100);
    std::vector<cv::Point2f> points4;
    int count4 = detector.detect_features(test_image, points4);
    EXPECT_GE(count4, 0);
}

// Test boundary values for set_ksize
TEST_F(FeatureDetectorTest, SetKsize_BoundaryValues) {
    // Should accept valid values
...
TEST_F(FeatureDetectorTest, SetKsize_ClampsToRange) {
    // Test clamping of 0 to 1 (lower bound)
...
TEST_F(FeatureDetectorTest, SetQualityLevel_Works) {
...
TEST_F(FeatureDetectorTest, SetMinDistance_Works) {
...
// Integration test: detect_features with custom settings
TEST_F(FeatureDetectorTest, DetectFeatures_WithCustomSettings) {
    ...
    std::vector<cv::Point2f> points;
    int count = detector.detect_features(test_image, points);
    
    EXPECT_GE(count, 0);
}

// Test that detector is available (always true with OpenCV implementation)
TEST_F(FeatureDetectorTest, IsAvailable) {
    bool available = detector.is_available();
    EXPECT_TRUE(available) << "FeatureDetector should always be available with OpenCV";
}

// Regression test: ensure setters don't crash and parameters are applied
TEST_F(FeatureDetectorTest, SettersRegressionTest) {
    ...
    std::vector<cv::Point2f> points2;
    EXPECT_NO_THROW(detector.detect_features(test_image, points2));
