...
TransformResult StabilizerCore::process_frame(frame_t* frame) {
    ...
    bool processing_success = ErrorHandler::safe_execute_bool([&]() -> bool {
        ...
        if (status_ == StabilizerStatus::INITIALIZING) {
            ...
            // Note: OpenCV dimensions are int but should be positive for valid frames
            if (current_gray.rows <= 0 || current_gray.cols <= 0) {
                char error_msg[256];
                snprintf(error_msg, sizeof(error_msg), 
                    UIConstants::ErrorMessages::INVALID_FRAME_DIMENSIONS,
                    current_gray.rows, current_gray.cols);
                ErrorHandler::log_error(ErrorCategory::FRAME_PROCESSING, "process_frame", error_msg);
                return false;
            }
            
            if (static_cast<uint32_t>(current_gray.rows) < StabilizerConstants::MIN_FEATURE_DETECTION_SIZE ||
                static_cast<uint32_t>(current_gray.cols) < StabilizerConstants::MIN_FEATURE_DETECTION_SIZE) {
                ErrorHandler::log_warning(ErrorCategory::FRAME_PROCESSING, "process_frame",
