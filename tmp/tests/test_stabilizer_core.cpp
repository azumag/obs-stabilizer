...
#include "../../src/core/stabilizer_constants.hpp"
#include "../../src/core/ui_constants.hpp"
...
struct MockOBSFrame {
    ...
    MockOBSFrame(uint32_t w, uint32_t h, cv::Mat& source_image)
        : width(w), height(h), format(StabilizerConstants::VIDEO_FORMAT_I420) {
...
TEST_F(StabilizerCoreTest, FeatureDetectionTest) {
    ...
    cv::goodFeaturesToTrack(test_frame_1, detected_points,
                           std::min(config.max_features, 1000), 0.01, 10);

    EXPECT_GT(detected_points.size(), 0) << "No features detected in test frame";
    EXPECT_LE(detected_points.size(), static_cast<size_t>(config.max_features))
...
TEST_F(StabilizerCoreTest, TransformSmoothingTest) {
    ...
    // Note: This test would need access to internal transform history
    // For now, test the smoothing concept with manual calculation
...
TEST_F(StabilizerCoreTest, TransformHistoryLimitTest) {
    ...
    // Test the concept of transform history limiting
    std::vector<cv::Mat> test_history;
    
    for (int i = 0; i < max_history * 2; i++) {
        cv::Mat transform = cv::Mat::eye(2, 3, CV_64F);
        transform.at<double>(0, 2) = static_cast<double>(i);
        test_history.push_back(transform.clone());

        // Limit history size
        if (test_history.size() > static_cast<size_t>(max_history)) {
            test_history.erase(test_history.begin());
        }

        EXPECT_LE(test_history.size(), static_cast<size_t>(max_history))
            << "Transform history exceeds maximum size";
    }

    EXPECT_EQ(test_history.size(), static_cast<size_t>(max_history))
...
TEST_F(StabilizerCoreTest, NegativeFrameDimensionValidationTest) {
    // Test handling of frames with negative dimensions (possible frame acquisition failure)
    // This directly tests the validation logic in stabilizer_core.cpp:107-114
    
    // Create mock frames with invalid dimensions to simulate frame acquisition failure
    cv::Mat negative_rows_frame = cv::Mat::zeros(0, 640, CV_8UC1);  // Zero rows (equivalent to negative)
    cv::Mat negative_cols_frame = cv::Mat::zeros(480, 0, CV_8UC1);  // Zero cols (equivalent to negative)
    cv::Mat empty_frame;  // Completely empty frame
    
    // Create mock OBS frames for testing - these represent corrupted/failed frame acquisition
    MockOBSFrame mock_negative_rows(640, 480, negative_rows_frame);
    MockOBSFrame mock_negative_cols(640, 480, negative_cols_frame);
    MockOBSFrame mock_empty(640, 480, empty_frame);
    
    // Test that process_frame correctly rejects frames with invalid dimensions
    
    // 1. Test frame with zero/negative rows
    TransformResult result1 = stabilizer->process_frame(reinterpret_cast<frame_t*>(&mock_negative_rows));
    EXPECT_FALSE(result1.success) << "process_frame should fail for frame with zero/negative rows";
    EXPECT_EQ(stabilizer->get_status(), StabilizerStatus::ERROR_RECOVERY) 
        << "Status should indicate error recovery after invalid frame";
    
    // Reset stabilizer for next test
    stabilizer->reset();
    stabilizer->initialize(config);
    
    // 2. Test frame with zero/negative cols  
    TransformResult result2 = stabilizer->process_frame(reinterpret_cast<frame_t*>(&mock_negative_cols));
    EXPECT_FALSE(result2.success) << "process_frame should fail for frame with zero/negative cols";
    EXPECT_EQ(stabilizer->get_status(), StabilizerStatus::ERROR_RECOVERY)
        << "Status should indicate error recovery after invalid frame";
    ...
    // 3. Test completely empty frame
    TransformResult result3 = stabilizer->process_frame(reinterpret_cast<frame_t*>(&mock_empty));
    EXPECT_FALSE(result3.success) << "process_frame should fail for completely empty frame";
    EXPECT_EQ(stabilizer->get_status(), StabilizerStatus::ERROR_RECOVERY)
        << "Status should indicate error recovery after empty frame";
    
    // Verify that all error cases properly set the transform result as unsuccessful
    EXPECT_FALSE(result1.transform_matrix.is_valid()) << "Invalid frame should not produce valid transform";
    EXPECT_FALSE(result2.transform_matrix.is_valid()) << "Invalid frame should not produce valid transform";
    EXPECT_FALSE(result3.transform_matrix.is_valid()) << "Invalid frame should not produce valid transform";
}